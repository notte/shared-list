...
export const getListInvites = cache(
  async (listId: string): Promise<GetListInvitesResponse | null> => {
    ...
    const cached = unstable_cache(
      async (): Promise<GetListInvitesResponse | null> => {
        try {
...
export const getListMembers = cache(
  async (listId: string): Promise<GetListMembersResponse | null> => {
    ...
    const cached = unstable_cache(
      async (): Promise<GetListMembersResponse | null> => {
        try {
          const membersRef = db
            .collection("lists")
            .doc(listId)
      ...
      async (): Promise<GetInviteCodeDetailResponse | null> => {
        try {
          ...
          return {
            ...
            createdAt:
              typeof inviteData.createdAt?.toDate === "function"
